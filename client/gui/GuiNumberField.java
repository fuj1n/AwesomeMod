...
public class GuiNumberField extends Gui {
	...
	protected boolean decrement() {
		if (!this.getText().isEmpty()) {
			if (Integer.parseInt(this.getText()) > minNumber || !hasMin) {
	...
	protected boolean increment() {
		if (!this.getText().isEmpty()) {
			if (Integer.parseInt(this.getText()) < maxNumber || !hasMax) {
				int value = Integer.parseInt(this.getText()) + 1;
				this.setText(Integer.toString(value));
				return true;
			} else {
			...
			this.setText(Integer.toString(minNumber + 1));
		}
		return false;
	}
				...
				default:
					if (ChatAllowedCharacters.isAllowedCharacter(par1)) {
						if (Character.isDigit(par1)) {
							...
							if (Integer.parseInt(text) > maxNumber && hasMax) {
								text = Integer.toString(maxNumber);
							}
							if (Integer.parseInt(text) < minNumber && hasMin) {
								text = Integer.toString(minNumber);
							}
						}
						return true;
					} else {
