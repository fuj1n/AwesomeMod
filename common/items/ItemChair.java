...
public class ItemChair extends Item {
	...
	public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10) {
		...
		if (par1ItemStack.stackSize == 0) {
			...
			return false;
		} else if (par5 == 255 && Block.blocksList[beginBlockID].blockMaterial.isSolid()) {
			return false;
		} else if (par3World.canPlaceEntityOnSide(beginBlockID, par4, par5, par6, false, par7, par2EntityPlayer, par1ItemStack)) {
			Block block = Block.blocksList[beginBlockID];
			int j1 = this.getMetadata(par1ItemStack.getItemDamage());
			int k1 = Block.blocksList[beginBlockID].onBlockPlaced(par3World, par4, par5, par6, par7, par8, par9, par10, j1);
	...
	public boolean placeBlockAt(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side, float hitX, float hitY, float hitZ, int metadata) {
		int i3 = MathHelper.floor_double((player.rotationYaw * 4.0F / 360.0F) + 0.5D) & 3;
		int id = beginBlockID;
		switch (i3) {
		case 0:
			id += 2;
